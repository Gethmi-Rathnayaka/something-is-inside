...
public class MarieLogic : DollBase
{
    ...
    {
        // Marie: no mood change on ignore, but corruption builds over 5 days
        daysFullyIgnored++;
        // Note: interactedToday stays false → neglect counter still increments
        visuals?.UpdateVisuals(state);
    ...
    protected override void ProcessNightEvent()
    {
        ...
        {
            daysFullyIgnored++;
            // Incremental corruption: +2 corruption per ignored day
            state.ModifyCorruption(2);
            Debug.Log($"[Marie] Ignored day #{daysFullyIgnored} — +2 corruption.");
        }
        else
        {
            daysFullyIgnored = 0;
        ...
        // ── After 5 consecutive ignored days → extra corruption gain ──
        if (daysFullyIgnored >= 5)
        {
            state.ModifyCorruption(10);
            Debug.Log("[Marie] 5 ignored days — +10 bonus corruption added.");
        }
